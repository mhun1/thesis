...
def res2net50_v1b_26w_4s(pretrained=False, map_location=None, **kwargs):
    """Constructs a Res2Net-50_v1b_26w_4s model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    ...
    if pretrained:
        if map_location == None:
            model.load_state_dict(
                model_zoo.load_url(model_urls["res2net50_v1b_26w_4s"])
            )
        else:
            model.load_state_dict(
                torch.load(
                    "/content/colab_trainer/backbone/res2net50_v1b_26w_4s-3cf99910.pth"
                )
            )
    return model
